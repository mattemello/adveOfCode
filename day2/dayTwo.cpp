...
bool isGreat(int* number, int iPos, int jPos, bool alredyIn) {

    if(number[iPos] > number[jPos]){
        if(number[iPos] - number[jPos] > 3){
            if(alredyIn){
                return false;
            }
            return isGreat(number, iPos - 1, jPos, true) || isGreat(number, iPos, jPos + 1, true);
        }
        return true;
    }else{
        if(alredyIn){
...
bool isNGreat(int* number, int iPos, int jPos, bool alredyIn) {

    if(number[iPos] < number[jPos]){
        if(number[jPos] - number[iPos] > 3){
            if(alredyIn){
                return false;
            }
            return isGreat(number, iPos - 1, jPos, true) || isGreat(number, iPos, jPos + 1, true);
...
bool isGood2(int* number, int dimension){
    ...
    bool increase = true;
    for(int i = 1; i < dimension; i++) {
        if(!isGreat(number, i-1, i, false)) {
            std::cout << "is great -> " << number[i-1] << " " << number[i] << std::endl;
            increase = false;
        }
    }
    ...
    bool decrease = true;
    for(int i = 1; i < dimension; i++) {
        if(!isNGreat(number, i-1, i, false)) {
            std::cout << "is Not great -> " << number[i-1] << " " << number[i] << std::endl;
            decrease = false;
        }
    }
    ...
    if(decrease){
            std::cout << "is decrease" << std::endl;
        
    }
    if(increase){
            std::cout << "is increase" << std::endl;
